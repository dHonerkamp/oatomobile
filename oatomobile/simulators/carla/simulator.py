...
class SEMANTIC_LIDARSensor(simulator.Sensor):
    ...
    def get_observation(
        ...
        try:
            ...
            instance_obs = cutil.carla_semantic_lidar_measurement_to_instance_ndarray(data, self._m, self._disp_size,
                                                                                      semantic_camera_obs,
                                                                                      depth_camera_obs)
            # [320, 180, 7]
...
class CARLASimulator(simulator.Simulator):
    ...
    def close(self) -> None:
        ...
        self._client.apply_batch(
            [carla.command.DestroyActor(x) for x in [self._hero] + self._vehicles + self._pedestrians])
